...
def get_last_processed_time(**kwargs):
    with Session_Source() as session_source:
        state = (
            session_source.query(MachineData)
...
def request_for_detection(**kwargs):
    ...
    with Session_Source() as session_source:
        new_data_statement = (
            session_source.query(MachineData)
            .filter(MachineData.timestamp > kwargs["last_processed_time"])
            .statement
        )
    ...
    response = requests.post(
        url=api_url, json=x, headers={"Content-Type": "application/json"}
    ).json()
...
def insert_event_data(**kwargs):
    ...
    for time, pred in zip(times, predictions):
        data = MachinePredict(
            source_time=datetime.datetime.strptime(time, "%Y-%m-%d %X")
...
with DAG(
    ...
    schedule_interval=datetime.timedelta(seconds=30),  # Schedule Trigger
    catchup=False,
) as dag:
    ...
    anomaly_request = PythonOperator(
        ...
        provide_context=True,
        op_kwargs={
            "last_processed_time": '{{ task_instance.xcom_pull(task_ids="get_last_processed_time") }}'
        },
        dag=dag,
    )
